...
export const AutoChart = (props: AutoChartProps) => {
  ...
  useEffect(() => {
    if (stableData && advisor) {
      // 调试：输出数据结构
      console.log('[AutoChart] Data received:', stableData);
      console.log('[AutoChart] ChartType from backend:', chartType);
      
      const avaAdvices = getVisAdvices({
        data: stableData,
        myChartAdvisor: advisor,
      });
      console.log('[AutoChart] AVA advices:', avaAdvices);
      
      // 合并模型推荐的图表类型和 ava 推荐的图表类型
      const allAdvices = getMergedAdvices(avaAdvices);
      console.log('[AutoChart] Merged advices:', allAdvices);
      
      setAdvices(allAdvices);
      // 仅在首次初始化或用户未手动选择时设置默认图表类型
      if (!isUserSelectedRef.current) {
        setRenderChartType(allAdvices[0]?.type as ChartType);
      }
    }
  }, [stableData, advisor, chartType]);
  ...
  const visComponent = useMemo(() => {
    ...
    if (advices?.length > 0) {
      ...
      if (spec) {
        if (stableData && spec.data && ['line_chart', 'step_line_chart'].includes(chartTypeInput)) {
          // 处理 ava 内置折线图的排序问题
          const dataAnalyzerOutput = advisor?.dataAnalyzer.execute({ data: stableData });
