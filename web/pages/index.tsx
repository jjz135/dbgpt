...
import { STATIC_DISPLAY_NAME } from '@/utils/constants';
...
const Playground: NextPage = () => {
  ...
  const getAppListWithParams = async (params: Record<string, string>): Promise<[] | AppListResponse> => {
  ...
  const getHotAppList = async (params: Record<string, string>): Promise<[] | AppListResponse> => {
    const [, data] = await apiInterceptors(
      recommendApps({
        page_no: '1',
        page_size: '12',
  ...
  const { run: getAppListFn, loading } = useRequest(
    async (app_name = '', page_no = '1', page_size = '12'): Promise<[] | AppListResponse> => {
      switch (activeKey) {
        case 'recommend':
    ...
    },
    {
      ...
      onSuccess: (res: [] | AppListResponse) => {
        if (activeKey === 'recommend') {
          ...
          if ('app_list' in res) {
            const code = res?.app_list?.[0]?.app_code;
  ...
  function loadMoreRows({ startIndex, stopIndex }: IndexRange): Promise<void> {
    const pageSize = 12;
    ...
    return new Promise((resolve) => {
      getAppListFn('', currentPage.toString());
      resolve();
    });
  }
          ...
          LeftBottom={
            <div className='flex gap-8 items-center text-[#878c93] text-sm dark:text-stone-200'>
              <div className='flex gap-1 items-center'>
                <Avatar className='bg-gradient-to-tr from-[#31afff] to-[#1677ff] cursor-pointer'>{STATIC_DISPLAY_NAME}</Avatar>
                <span>{STATIC_DISPLAY_NAME}</span>
              </div>
              {activeKey === 'recommend' ? (
