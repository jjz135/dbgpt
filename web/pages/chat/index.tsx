...
const Chat: React.FC = () => {
  ...
  useEffect(() => {
    const resourceBindValues =
      appInfo?.param_need?.filter(item => item.type === 'resource').map(item => item.bind_value).filter(Boolean) || [];
    ...
    setResourceValue(
      knowledgeId || dbName || (scene === 'chat_flow' ? resourceBindValues : resourceBindValues[0]),
    );
  }, [appInfo, dbName, knowledgeId, model, scene]);
