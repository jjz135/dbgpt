...
const ChatCompletion: React.FC = () => {
  ...
  useAsyncEffect(async () => {
    ...
    if (initMessage && initMessage.id === chatId) {
      ...
      if (res) {
        ...
        const resourceParams = res?.param_need?.filter(item => item.type === 'resource') || [];
        const resourceBindValues = resourceParams.map(item => item.bind_value).filter(Boolean);
        const resource = scene === 'chat_flow' ? resourceBindValues : resourceBindValues[0];
        ...
        await handleChat(initMessage.message, {
          ...
          ...(paramKey.includes('resource') &&
            scene === 'chat_flow' && {
              ext_info: {
                bound_resources: Array.isArray(resource) ? resource : resource ? [resource] : [],
              },
            }),
          ...(paramKey.includes('resource') &&
            scene !== 'chat_flow' && {
  ...
  }, [chatId, currentDialogInfo, model, scene]);
