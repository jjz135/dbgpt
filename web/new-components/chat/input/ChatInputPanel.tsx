...
const ChatInputPanel: React.ForwardRefRenderFunction<any, { ctrl: AbortController }> = ({ ctrl }, ref) => {
  ...
  const onSubmit = async () => {
    ...
    setUserInput('');
    const isFlowScene = scene === 'chat_flow';
    const resources = isFlowScene ? [] : parseResourceValue(resourceValue);
    // Clear the resourceValue if it not empty
    let newUserInput: UserChatContent;
    if (resources.length > 0) {
      if (scene !== 'chat_excel' && !isFlowScene) {
    ...
    const boundResources = Array.isArray(resourceValue)
      ? resourceValue.filter(Boolean)
      : resourceValue
        ? [resourceValue]
        : [];
    ...
    if (paramKey.includes('resource')) {
      if (isFlowScene) {
        ...
        params.select_param =
          typeof resourceValue === 'string' ? resourceValue : JSON.stringify(resourceValue) || currentDialogue.select_param;
      }
    }
