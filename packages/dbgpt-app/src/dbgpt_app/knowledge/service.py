...
class KnowledgeService:
    ...
    def update_knowledge_space(
        ...
        """
        # Get the old space info to check if name is changing
        old_space = knowledge_space_dao.get_one({"id": space_id})
        if not old_space:
            raise Exception(f"Knowledge space with id {space_id} not found")
        
        old_name = old_space.name
        new_name = space_request.name
        
        # If name is being changed, we need to sync related data
        if new_name and old_name != new_name:
            logger.info(f"Renaming knowledge space from '{old_name}' to '{new_name}'")
            
            # Update document table's space field
            try:
                session = knowledge_document_dao.get_raw_session()
                updated_count = session.query(KnowledgeDocumentEntity).filter(
                    KnowledgeDocumentEntity.space == old_name
                ).update({KnowledgeDocumentEntity.space: new_name})
                session.commit()
                session.close()
                logger.info(f"Updated {updated_count} documents from space '{old_name}' to '{new_name}'")
            except Exception as e:
                logger.error(f"Failed to update documents space field: {e}")
        ...
        logger.info(f"Successfully updated knowledge space id={space_id}, name='{new_name}'")
