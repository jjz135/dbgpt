...
@router.post("/knowledge/space/update")
async def space_update(request: KnowledgeSpaceRequest):
    """Update knowledge space information including name"""
    logger.info(f"/space/update params: {request}")
    try:
        if request.id is None:
            return Result.failed(code="E000X", msg="space id is required")
        
        # Get current space info
        current_spaces = await blocking_func_to_async(
            get_executor(), 
            knowledge_space_service.get_knowledge_space,
            KnowledgeSpaceRequest(id=request.id)
        )
        
        if not current_spaces or len(current_spaces) == 0:
            return Result.failed(code="E000X", msg=f"Space with id {request.id} not found")
        
        current_space = current_spaces[0]
        old_name = current_space.name
        new_name = request.name
        
        # If name is being changed, validate it
        if new_name and new_name != old_name:
            # Check if new name already exists
            existing_spaces = await blocking_func_to_async(
                get_executor(),
                knowledge_space_service.get_knowledge_space,
                KnowledgeSpaceRequest(name=new_name)
            )
            if existing_spaces and len(existing_spaces) > 0:
                return Result.failed(
                    code="E000X", 
                    msg=f"Knowledge space with name '{new_name}' already exists"
                )
            
            # Update vector store index name if needed
            if current_space.vector_type:
                try:
                    storage_manager = StorageManager.get_instance(CFG.SYSTEM_APP)
                    storage_connector = storage_manager.get_storage_connector(
                        index_name=old_name, 
                        storage_type=current_space.vector_type
                    )
                    # Rename the vector store
                    if hasattr(storage_connector, 'client'):
                        # For Chroma and similar stores
                        if hasattr(storage_connector.client, 'get_or_create_collection'):
                            # Get old collection
                            old_collection = storage_connector.client.get_collection(old_name)
                            if old_collection:
                                # Create new collection with new name
                                new_collection = storage_connector.client.get_or_create_collection(new_name)
                                # Copy data from old to new
                                old_data = old_collection.get(include=['embeddings', 'documents', 'metadatas'])
                                if old_data['ids']:
                                    new_collection.add(
                                        ids=old_data['ids'],
                                ...
                                storage_connector.client.delete_collection(old_name)
                                logger.info(f"Successfully renamed vector store from '{old_name}' to '{new_name}'")
                except Exception as e:
                    logger.warning(f"Failed to rename vector store: {e}. Continuing with space name update.")
        
        # Update space in database
        await blocking_func_to_async(
            get_executor(),
            knowledge_space_service.update_knowledge_space,
            request.id,
            request
        )
        
        logger.info(f"Successfully updated space {request.id}")
        return Result.succ({"message": "Space updated successfully"})
    except Exception as e:
        logger.exception(f"Space update error!{str(e)}")
        return Result.failed(code="E000X", msg=f"space update error {e}")
