...
class KnowledgeApiClient(ApiClient):
    ...
    def space_update(self, request: KnowledgeSpaceRequest):
        """Update knowledge space name or other properties"""
        return self._post("/knowledge/space/update", data=request)
...
def knowledge_rename(
    api_address: str, old_name: str, new_name: str, confirm: bool = False
):
    """Rename a knowledge space
    
    Args:
        api_address: API server address
        old_name: Current knowledge space name
        new_name: New knowledge space name
        confirm: Skip confirmation if True
    """
    client = KnowledgeApiClient(api_address)
    
    # Check if old space exists
    space_list = client.space_list(KnowledgeSpaceRequest(name=old_name))
    if not space_list:
        raise Exception(f"No knowledge space found with name: {old_name}")
    
    # Check if new name already exists
    existing_space = client.space_list(KnowledgeSpaceRequest(name=new_name))
    if existing_space:
        raise Exception(f"Knowledge space with name '{new_name}' already exists")
    
    # Confirm with user
    if not confirm:
        user_input = (
            input(
                f"Are you sure you want to rename knowledge space "
                f"'{old_name}' to '{new_name}'? Type 'yes' to confirm: "
            )
            .strip()
            .lower()
        )
        if user_input != "yes":
            logger.warning("Rename operation cancelled.")
            return
    
    # Get space ID
    space_info = space_list[0]
    space_id = space_info.get("id")
    
    # Update space name
    update_request = KnowledgeSpaceRequest()
    update_request.id = space_id
    update_request.name = new_name
    
    try:
        client.space_update(update_request)
        logger.info(
            f"Successfully renamed knowledge space from '{old_name}' to '{new_name}'"
        )
    except Exception as e:
        raise Exception(f"Failed to rename knowledge space: {str(e)}")
