...
class MultiAgents(BaseComponent, ABC):
    ...
    def get_or_build_agent_memory(self, conv_id: str, dbgpts_name: str) -> AgentMemory:
        ...
        # 最大线程限制16，单次最多chunks限制50
        vector_store._max_chunks_once_load = 50
        vector_store._max_threads = 16
    ...
    def _build_flow_history(
        ...
        for message in historical_dialogues or []:
            content = self._strip_flow_vis_blocks((message.content or "").strip())
            if not content:
                continue
            role = "助手"
            if getattr(message, "role", None) == "human":
                role = "用户"
            elif getattr(message, "role", None) == "assistant":
                role = "助手"
            elif getattr(message, "sender", None) == "human":
                role = "用户"
            history.append({"role": role, "content": content})
        history.append({"role": "用户", "content": current_user_query})
        return self._compress_flow_history_entries(history)

    def _build_flow_history_from_conversation(
        self, conversation: StorageConversation, max_items: int = 8
    ) -> List[Dict[str, str]]:
        history: List[Dict[str, str]] = []
        messages = getattr(conversation, "messages", []) or []
        for message in messages[-max_items:]:
            content = self._strip_flow_vis_blocks(
    ...
    def _strip_flow_vis_blocks(self, text: str) -> str:
        cleaned = re.sub(
            r"`{3,6}(?:vis-plugin|vis-thinking|agent-plans)\n[\s\S]*?\n`{3,6}",
            "",
            text,
        )
        cleaned = re.sub(r"\n{3,}", "\n\n", cleaned)
    ...
    def _extract_flow_key_sentence(self, text: str) -> str:
        sentences = [
            sentence.strip()
        ...
        if not sentences:
            return text.strip()
        preferred_keywords = ["暂无", "包含", "相关", "建议", "仅提供", "未提供", "适用范围"]
        for sentence in sentences:
            if any(keyword in sentence for keyword in preferred_keywords):
                return sentence
        return sentences[-1]

    def _compress_flow_history_entries(
        self, entries: List[Dict[str, str]], max_rounds: int = 3
    ) -> List[Dict[str, str]]:
        if not entries:
            return []
        recent = entries[-(max_rounds * 2) :]
        compressed: List[Dict[str, str]] = []
        for entry in recent:
            role = entry.get("role") or "助手"
            content = str(entry.get("content") or "").strip()
            if not content:
                continue
            if role == "助手":
                content = self._extract_flow_key_sentence(content)
                limit = 220
            else:
                limit = 180
    ...
    async def agent_chat_v2(
        ...
        **ext_info,
        ...
        if (
            ...
            flow_history = ext_info.get("conversation_history") or self._build_flow_history(
                user_query, historical_dialogues
            )
            flow_extra = {
                **ext_info,
                "parent_conv_uid": conv_id,
                "agent_conv_uid": agent_conv_id,
                "conversation_history": flow_history,
            }
    ...
    async def app_agent_chat(
        ...
        **ext_info,
        ...
        flow_history = self._build_flow_history_from_conversation(current_message)
        agent_conv_id = None
        agent_task = None
