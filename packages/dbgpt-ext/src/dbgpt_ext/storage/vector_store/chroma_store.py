...
class ChromaStore(VectorStoreBase):
    ...
    def __init__(
        ...
        # 确保 chroma_path 不为 None
        if not chroma_path:
            chroma_path = os.path.join(PILOT_PATH, "data")
        resolved_path = resolve_root_path(chroma_path)
        if not resolved_path:
            resolved_path = chroma_path
        self.persist_dir = os.path.join(resolved_path, "chromadb")
        ...
        logger.info(f"------------正在构建 Chroma collection: {self._collection_name}")
        logger.info(f"------------存储路径为: {self.persist_dir}")
    ...
    def delete_by_ids(self, ids, batch_size: int = 50):
