...
from dbgpt.vis.tags.vis_insight import Vis, VisInsight
...
class InsightAction(Action[List[ChartItem]]):
    """Insight action class."""

    def __init__(self, **kwargs):
        """Insight action init."""
        super().__init__(**kwargs)
        self._render_protocol = VisInsight()
        ...
        **kwargs,
        ...
        try:
            ...
            view = await self.render_protocol.display(
                charts=chart_params,
                content=json.dumps([chart_item.to_dict() for chart_item in chart_items], ensure_ascii=False)
            )
            return ActionOutput(
                is_exe_success=True,
                content=json.dumps(
            ...
            return ActionOutput(
                is_exe_success=False, content=f"Insight action run failed!{str(e)}"
            )
