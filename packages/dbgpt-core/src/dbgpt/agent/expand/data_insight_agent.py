...
class DataInsightAgent(ConversableAgent):
    """Data Insight Agent."""
    ...
    profile: ProfileConfig = ProfileConfig(
        name=DynConfig(
            "Visionary",
            category="agent",
            key="dbgpt_agent_expand_data_insight_agent_profile_name",
        ),
        role=DynConfig(
            "DataInsight",
            category="agent",
            key="dbgpt_agent_expand_data_insight_agent_profile_role",
        ),
        goal=DynConfig(
            "Read the provided historical messages, collect various analysis SQLs "
            "from them, and assemble them into professional reports.",
            category="agent",
            key="dbgpt_agent_expand_data_insight_agent_profile_goal",
        ),
        constraints=DynConfig(
            ...
            category="agent",
            key="dbgpt_agent_expand_data_insight_agent_profile_constraints",
        ),
        desc=DynConfig(
            "Observe and organize various analysis results and construct "
            "professional reports",
            category="agent",
            key="dbgpt_agent_expand_data_insight_agent_profile_desc",
        ),
    )
    ...
    def __init__(self, **kwargs):
        ...
        self._init_actions([InsightAction])
