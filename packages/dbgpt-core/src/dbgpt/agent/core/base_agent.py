...
class ConversableAgent(Role, Agent):
        ...
        try:
            ...
            while current_retry_counter < self.max_retry_count:
                if current_retry_counter > 0:
                    ...
                    # 将重试次数传递给agent
                    reply_message.context["retry_count"] = current_retry_counter
    ...
    def _print_received_message(self, message: AgentMessage, sender: Agent):
        ...
        verbose = self.not_null_agent_context.verbose if self.agent_context else False
        ...
        if verbose:
            ...
            # Simplified output - only show key transitions and status
            sender_name = sender.name if sender.name else sender.role
            receiver_name = self.name if self.name else self.role
            
            # Only print action status if present (most important info)
            action_report = message.action_report
            if action_report:
                status_symbol = "✓" if action_report.is_exe_success else "✗"
                status_msg = "success" if action_report.is_exe_success else "failed"
                print(
                    f"[{sender_name} → {receiver_name}] Action {status_symbol} ({status_msg})",
                    flush=True
                )
