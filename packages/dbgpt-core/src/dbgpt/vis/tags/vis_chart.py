...
class VisChart(Vis):
    ...
    def sync_generate_param(self, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        param["sql"] = sql
        chart_type = chart.get("display_type", "response_table")
        # 规范化未知或不合适的图表类型
        if chart_type in ["histogram", "response_histogram"]:
            chart_type = "response_bar_chart"
        # 默认弱化气泡图；除非有明确理由，否则回退为表格
        if chart_type == "response_bubble_chart":
